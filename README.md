# 功能计划
1. 信息采集从bangumi，tmdb等公开api（✅ 已实现bangumi和tmdb）
2. 在线视频弹幕播放，包括Animate4k等滤镜加强播放功能，和弹幕功能（✅ 已实现）
3. 字幕自动下载匹配（🔄 进行中 - 核心模块已实现，UI集成待完成）
4. aria2文件下载管理器（✅ 已实现）
5. 视频管理分类(播放历史记录和基于bangumi等api的播放喜好已经实现)（✅ 已实现）
6. 完整的多语言架构 (slang x riverpod x weblate)（✅ 已实现 - 支持中文、英语、日语、繁体中文）
...
### 贡献翻译
欢迎为 Kazumi 贡献翻译！详见 [国际化文档](docs/i18n.md)。
